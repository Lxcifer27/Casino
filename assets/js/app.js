...
function buildBettingBoard(){
	...
	let chipValues = [2, 5, 10, 50, 100, 200, 500, 1000, 3000, 5000,  'clear'];
...
function customSpin(winNum){
	var winningSpin = winNum;
	spinWheel(winningSpin);
	setTimeout(function(){
		if(numbersBet.includes(winningSpin)){
			let winValue = 0;
...
const elements = document.querySelectorAll('.cdChip');
	elements.forEach((element) => {
		element.classList.remove('clearBet');
	});

document.querySelectorAll('.cdChip')[10].classList.add('clearBet');